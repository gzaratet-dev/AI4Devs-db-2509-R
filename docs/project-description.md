...
**Tipo de Relaciones:**
- **One-to-Many:** Un candidato puede tener múltiples registros de educación, experiencia laboral y CVs
- **Foreign Keys:** Todas las relaciones usan `ON DELETE RESTRICT` y `ON UPDATE CASCADE`

### 4.3 Restricciones y Validaciones

- **Unique Constraints:**
  - `Candidate.email` es único (índice único)
- **Foreign Key Constraints:**
  - `Education.candidateId` → `Candidate.id`
  - `WorkExperience.candidateId` → `Candidate.id`
